...
def _render_watch_state_table(watch_state_df: pd.DataFrame) -> str:
    ...
    rows = []
    for _, row in df.iterrows():
        status_raw = str(row.get("status") or "watching")
        status_cn_map = {"watching": "观察中", "bought": "已买入", "exited": "已退出"}
        status_cn = status_cn_map.get(status_raw.lower(), status_raw)
        chip_cls = _status_tone(status_raw)
        pullback_cn = "是" if int(_safe_float(row.get("pullback_ready"))) > 0 else "否"
        chase_cn = "是" if int(_safe_float(row.get("chase_ok"))) > 0 else "否"
        rows.append(
            "<tr>"
            f"<td class='mono'>{html.escape(str(row.get('symbol') or '-'))}</td>"
            f"<td>{html.escape(str(row.get('name') or '-'))}</td>"
            f"<td><span class='status-chip {chip_cls}'>{html.escape(status_cn)}</span></td>"
            f"<td>{html.escape(str(row.get('sector_name') or '-'))}</td>"
            f"<td class='mono right'>{_safe_float(row.get('score')):.1f}</td>"
            f"<td class='mono'>{_fmt_trade_date(str(row.get('breakout_date') or '-'))}</td>"
            f"<td class='mono right'>{_safe_float(row.get('breakout_price')):.2f}</td>"
            f"<td class='mono right'>{_safe_float(row.get('stop_price')):.2f}</td>"
            f"<td class='mono right'>{_safe_float(row.get('ma10')):.2f}</td>"
            f"<td class='mono right'>{_safe_float(row.get('ma20')):.2f}</td>"
            f"<td class='mono center'>{pullback_cn}</td>"
            f"<td class='mono center'>{chase_cn}</td>"
            f"<td class='mono'>{_fmt_trade_date(str(row.get('last_signal_date') or '-'))}</td>"
            "</tr>"
        )

    return (
        '<div class="table-wrap watch-state-wrap">'
        '<table class="desk-table watch-state-table">'
        "<thead><tr>"
        "<th>代码</th><th>名称</th><th>状态</th><th>行业</th><th class='right'>评分</th>"
        "<th>突破日</th><th class='right'>突破价</th><th class='right'>止损位</th>"
        "<th class='right'>MA10</th><th class='right'>MA20</th><th class='center'>回踩</th><th class='center'>不追高</th><th>最近信号日</th>"
        "</tr></thead>"
...
<style>
    ...
    .watch-state-wrap {
        max-height: 460px;
        margin-top: 8px;
        background: linear-gradient(150deg, rgba(9, 22, 45, 0.92), rgba(8, 18, 36, 0.82));
        border-color: rgba(97, 132, 189, 0.42);
        box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.03), 0 12px 34px rgba(0, 0, 0, 0.28);
    }
    .watch-state-table thead th {
        background: linear-gradient(180deg, rgba(16, 35, 72, 0.98), rgba(14, 30, 62, 0.96));
    }
    .watch-state-table .center {
        text-align: center;
    }
    .status-chip {
        display: inline-flex;
        ...
        padding: 2px 8px;
        border-radius: 999px;
        border: 1px solid rgba(120, 149, 204, 0.42);
        font-size: 12px;
        line-height: 1.2;
        letter-spacing: 0.02em;
    ...
    .status-chip.chip-watching {
        border-color: rgba(63, 200, 255, 0.55);
        background: rgba(20, 83, 116, 0.30);
        color: #c7efff;
    }
    .status-chip.chip-bought {
        border-color: rgba(46, 203, 136, 0.68);
        background: rgba(22, 111, 79, 0.30);
        color: #c8ffe5;
    }
    .status-chip.chip-exited {
        border-color: rgba(255, 124, 138, 0.62);
        background: rgba(121, 39, 51, 0.28);
        color: #ffd6dc;
    }
    .diary-box {
        border: 1px solid var(--line);
...
trades_df = get_trades(OFFICIAL_PORTFOLIO_2_ID, days=20)
...
with b2:
    ...
    st.markdown(_render_watch_state_table(watch_state_df), unsafe_allow_html=True)
