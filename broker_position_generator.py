...
⚠️ **严格警告**：此步骤**只查以上3家机构**，中信建投/东方财富/方正中期的数据**不属于这里**，不要混入！

**输出要求**：
- 将3家数据按品种合并，计算每个品种的合计净持仓变化
- **净持仓变化分类规则**（必须严格遵守）：
  - 合计净持仓变化 > 0（正数） → 该品种属于**多头增仓**，放入多头列表
  - 合计净持仓变化 < 0（负数） → 该品种属于**空头增仓**，放入空头列表
  - **绝对不允许**把正数放进空头列表，也不允许把负数放进多头列表！
...
⚠️ **严格警告**：此步骤**只查以上3家反指标**，海通/东证/国泰君安的数据**不属于这里**，不要混入！

**重要**：只汇总品种和合计方向，**不记录具体期货商名称**（报告中反指标统称"散户聚集地"）。
**注意区分**（必须严格遵守）：
- 合计净持仓变化 > 0（正数）→ 反指标做多，放入做多列表
- 合计净持仓变化 < 0（负数）→ 反指标做空，放入做空列表
...
### 3. 反指标信号区分
- **反指标做多区块**：只放净持仓 > 0（正数）的品种（数字为正，红色）
- **反指标做空区块**：只放净持仓 < 0（负数）的品种（数字为负，绿色）
- ⛔ **严格禁止**：反指标板块中**绝对不能出现**海通期货、东证期货、国泰君安、乾坤期货、摩根大通的名字！这些是正指标机构，不属于反指标！
- ⛔ **严格禁止**：反指标板块不显示任何期货商名称，统称"散户聚集地"
...
- [ ] 5日累计：有资金金额（亿元）
- [ ] 品种去重（无重复）
- [ ] 机构板块：只有海通/东证/国泰君安的数据，无中信建投/东方财富/方正中期
- [ ] **机构多头增仓：只放净变化>0（正数）的品种；机构空头增仓：只放净变化<0（负数）的品种**
- [ ] 反指标不显示期货商名称（统称"散户聚集地"）
- [ ] **反指标板块中不出现海通/东证/国泰君安/乾坤/摩根大通的名字**
