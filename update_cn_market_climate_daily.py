...
def normalize_market_amount_frames(sse: pd.DataFrame, szse: pd.DataFrame) -> pd.DataFrame:
    ...
    for raw, exchange in ((sse, "SSE"), (szse, "SZSE")):
        ...
        frames.append(frame)
    if len(frames) != 2:
        return pd.DataFrame(columns=["trade_date", "amount_yuan"])
    combined = pd.concat(frames, ignore_index=True)
    pivot = combined.pivot_table(
        index="trade_date", columns="exchange_id", values="amount", aggfunc="last"
    )
    if not {"SSE", "SZSE"}.issubset(pivot.columns):
        return pd.DataFrame(columns=["trade_date", "amount_yuan"])
    common = pivot[["SSE", "SZSE"]].dropna().sum(axis=1).rename("amount_yi").reset_index()
    common["amount_yuan"] = common["amount_yi"] * 100_000_000
    return common[["trade_date", "amount_yuan"]].sort_values("trade_date")
...
def fetch_market_amount_history(start_date: str, end_date: str) -> pd.DataFrame:
    token = os.getenv("TUSHARE_TOKEN")
    if not token:
        raise RuntimeError("成交额汇总需要 TUSHARE_TOKEN；融资数据本身不使用 Tushare")
...
def load_source_frames(engine: Any, *, start_date: str, end_date: str) -> dict[str, Any]:
    ...
    float_mv = _read_sql_or_empty(
        engine,
        """
        SELECT REPLACE(REPLACE(trade_date, '-', ''), '/', '') AS trade_date,
               SUM(circ_mv) * 10000 AS float_mv_yuan
        FROM stock_valuation
        WHERE REPLACE(REPLACE(trade_date, '-', ''), '/', '') BETWEEN :start_date AND :end_date
          AND (ts_code LIKE '%.SH' OR ts_code LIKE '%.SZ')
          AND circ_mv IS NOT NULL AND circ_mv > 0
        GROUP BY REPLACE(REPLACE(trade_date, '-', ''), '/', '')
        ORDER BY trade_date
        """,
        {"start_date": start_date, "end_date": end_date},
    )
    ...
    return {"margin": margin, "float_mv": float_mv, "index": index_prices, "cn10y": cn10y}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    three_year_start = (pd.to_datetime(backfill_start or end_date) - pd.DateOffset(years=3)).strftime("%Y%m%d")
    calendar_start = backfill_start or three_year_start
    trading_dates = load_trading_dates(engine, calendar_start, end_date)
    ...
    existing_margin = _read_sql_or_empty(
        ...
        WHERE trade_date BETWEEN :start_date AND :end_date
        """,
        {"start_date": calendar_start, "end_date": end_date},
    )
    fetched_margin, warnings = fetch_margin_records(
        client,
    ...
    sources = load_source_frames(engine, start_date=three_year_start, end_date=end_date)
    ...
    sources["amount"] = fetch_market_amount_history(three_year_start, end_date)

    basis_years = max(1, math.ceil((pd.to_datetime(end_date) - pd.to_datetime(calendar_start)).days / 365.25) + 1)
