...
COMPANY_ANALYSIS_KEYWORDS = (
    ...
    "价值投资", "长期持有", "长期投资", "长期价值", "安全边际", "护城河", "盈利质量",
    "现金流", "竞争力", "内在价值", "值不值得长期", "长期能买吗",
)
...
def is_pure_option_data_query(prompt_text: str) -> bool:
    return is_pure_option_data_request(prompt_text)
...
def is_us_option_market_profile_query(prompt_text: str) -> bool:
    ...
    task_type = classify_analysis_task_type(text).task_type
    if task_type == TASK_TYPE_STOCK_SELECTION:
        return False
    if task_type in {TASK_TYPE_OPTION_STRATEGY_WITH_SUBJECT, TASK_TYPE_OPTION_STRATEGY_NEEDS_SUBJECT} and not is_pure_option_data_query(text):
        return False
...
def is_market_data_query(prompt_text: str) -> bool:
    ...
    # 窄口径数据先确认；高层策略、筛选和分析任务不允许被数据词覆盖。
    if is_pure_option_data_query(prompt_text):
        return True

    task_policy = classify_analysis_task_type(prompt_text)
    if task_policy.task_type != TASK_TYPE_NORMAL:
        return False
