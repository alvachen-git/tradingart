...
FOLLOWUP_CONTEXT_PREFIXES = ("对，", "对,", "那", "再", "继续")
EXECUTE_SUGGESTED_ACTION_GOAL = "execute_suggested_action"
EXECUTE_SUGGESTED_ACTION_HINTS = (
    "好，帮我筛选",
    "好帮我筛选",
    "帮我筛选",
    "帮我筛一下",
...
COMMON_MARKET_ENTITIES = (
    "创业板ETF", "创业板指", "创业板指数", "创业板",
    "科创50ETF", "科创板50", "科创50",
    "沪深300ETF", "沪深300", "中证500ETF", "中证500", "中证1000", "上证50ETF", "上证50",
...
EXPLICIT_FINANCIAL_SUBJECT_KEYWORDS = (
    "美股", "A股", "a股", "港股", "日股", "欧股", "科创50", "沪深300", "中证500", "中证1000",
    "上证", "深成指", "创业板", "恒生", "恒科", "螺纹钢", "螺纹", "热卷", "铁矿", "焦煤", "焦炭",
    "纯碱", "玻璃", "甲醇", "豆粕", "菜粕", "棕榈油", "豆油", "橡胶", "PTA", "pta", "PVC", "pvc",
    "沪铜", "沪铝", "沪锌", "沪镍", "沪银", "沪金", "美元", "美债", "人民币", "ETF", "etf",
)
...
def looks_like_execute_suggested_action(prompt_text: str) -> bool:
    text = str(prompt_text or "").strip().lower()
    if not text:
        return False
    compact = re.sub(r"\s+", "", text)
    if len(compact) <= 12 and any(hint in text or hint in compact for hint in EXECUTE_SUGGESTED_ACTION_HINTS):
        return True
    has_short_action = len(text) <= 18 and any(hint in text for hint in ("筛选", "名单", "执行", "照做"))
    has_accept_prefix = text.startswith(("好", "行", "可以", "那", "嗯", "对"))
    return bool(has_short_action and has_accept_prefix)


def extract_suggested_actions(text: str, *, max_actions: int = 3) -> list[str]:
    ...
    actions: list[str] = []
    for chunk in chunks:
        if not any(hint in chunk for hint in SUGGESTED_ACTION_HINTS):
            continue
        if not any(hint in chunk for hint in STOCK_SELECTION_CONTEXT_HINTS):
            continue
        summary = summarize_anchor_text(chunk, max_chars=120)
...
def build_followup_action_context(anchor: dict[str, Any] | None) -> str:
    if not isinstance(anchor, dict) or not anchor:
        return ""
    actions = [str(item).strip() for item in (anchor.get("suggested_actions") or []) if str(item).strip()]
    if not actions:
        return ""
    lines = ["【上一轮可执行建议】"]
    user_query = summarize_anchor_text(str(anchor.get("user_query") or ""), max_chars=120)
    assistant_summary = summarize_anchor_text(str(anchor.get("assistant_summary") or ""), max_chars=180)
    if user_query:
        lines.append(f"- 上一轮问题：{user_query}")
    if assistant_summary:
        lines.append(f"- 上一轮回答摘要：{assistant_summary}")
    for action in actions[:3]:
        lines.append(f"- 建议动作：{action}")
    if any(any(hint in action for hint in STOCK_SELECTION_CONTEXT_HINTS) for action in actions):
        lines.append("- 动作类型：stock_selection")
    return "\n".join(lines)
...
def infer_followup_goal(prompt_text: str, *, recent_context: str = "", recent_focus_topic: str = "") -> str:
    ...
    if looks_like_execute_suggested_action(text):
        return EXECUTE_SUGGESTED_ACTION_GOAL
...
def infer_followup_intent(prompt_text: str) -> bool:
    ...
    has_explicit_subject = has_explicit_financial_subject(text)
    has_explicit_followup_cue = has_followup_cue(text)
    ...
    if len(text) <= 16 and has_action and has_detail:
        return True
    if followup_goal and (has_request_prefix or (len(text) <= 18 and not has_explicit_subject) or has_explicit_followup_cue):
        return True
    return False
...
def should_preserve_recent_context(
    ...
    *,
    ...
    if (
        ...
        and not has_explicit_followup_cue
        and not (semantic_related and is_same_domain)
        and not infer_correction_intent(
        text,
...
def extract_focus_entity(text: str) -> str:
    ...
    for suffix in FOCUS_ENTITY_SUFFIXES:
        doubled_suffix = suffix + suffix
        if candidate.endswith(doubled_suffix) and len(candidate) > len(doubled_suffix):
...
def has_explicit_financial_subject(text: str) -> bool:
    raw = str(text or "").strip()
    if not raw:
        return False
    if extract_focus_entity(raw):
        return True
    if any(keyword in raw for keyword in EXPLICIT_FINANCIAL_SUBJECT_KEYWORDS):
        return True
    if re.search(r"(?<!\d)\d{6}(?:\.(?:SH|SZ|BJ))?(?!\d)", raw, flags=re.I):
        return True
    if US_TICKER_PATTERN.search(raw):
        return True
    return False


def has_followup_cue(text: str) -> bool:
    raw = str(text or "").strip().lower()
    if not raw:
        return False
    if any(keyword in raw for keyword in FOLLOWUP_KEYWORDS):
        return True
    if any(raw.startswith(prefix) for prefix in FOLLOWUP_CONTEXT_PREFIXES):
        return True
    if any(raw.startswith(hint) for hint in FOLLOWUP_CONNECTOR_HINTS):
        return True
    if any(hint in raw for hint in FOCUS_PRONOUN_HINTS):
        return True
    return False
...
def build_topic_anchors(messages: Iterable[dict], max_anchors: int = 3) -> list[dict[str, Any]]:
    ...
    for idx, raw_msg in enumerate(messages):
        ...
        suggested_actions = extract_suggested_actions(assistant_text)
        context_limit = 900 if suggested_actions else 260
...
def _anchor_goal_score(anchor: dict[str, Any], followup_goal: str) -> float:
    ...
    if goal == EXECUTE_SUGGESTED_ACTION_GOAL:
        actions = anchor.get("suggested_actions") or []
        if actions:
            return 0.55
        return 0.20 if any(keyword in text for keyword in SUGGESTED_ACTION_HINTS) else 0.0
...
def select_target_anchor(
    ...
    *,
    ...
    if followup_goal == EXECUTE_SUGGESTED_ACTION_GOAL:
        action_anchors = [anchor for anchor in reversed(anchor_list) if anchor.get("suggested_actions")]
        if action_anchors:
            top_anchor = action_anchors[0]
            top_score = max(top_score, 1.25)
    ...
    clarify = ""
    if followup_goal == EXECUTE_SUGGESTED_ACTION_GOAL:
        action_anchors = [anchor for anchor in candidate_anchors if anchor.get("suggested_actions")]
        if len(action_anchors) >= 2 and len(prompt_raw) <= 8 and any(hint in prompt_text_lower for hint in ("按这个", "照做", "执行")):
            latest = action_anchors[-1]
            previous = action_anchors[-2]
            ambiguous = True
            clarify = (
                f"你是要按刚才“{summarize_anchor_text(latest.get('user_query', ''), 18)}”里的建议执行，"
                f"还是前面“{summarize_anchor_text(previous.get('user_query', ''), 18)}”那条？"
            )
            top_anchor = latest
