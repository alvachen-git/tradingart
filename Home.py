...
def native_share_button(text_content, key):
    ...
    <div id="{container_id}" style="
        ...
        line-height: 1.6;
        width: 400px; /* 固定宽度以保证生成图片排版一致 */
        position: fixed; top: -9999px; left: -9999px; /* 移出屏幕外，不可见但可截图 */
    ">
        <div style="display: flex; align-items: center; margin-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 15px;">
            ...
            <div>
                <div style="font-weight: 900; font-size: 18px; color: #fff;">爱波塔 AI</div>
                <div style="font-size: 12px; color: #94a3b8;">智能量化投资顾问</div>
            </div>
        </div>

        <div style="font-size: 15px; color: #cbd5e1; white-space: pre-wrap; margin-bottom: 20px;">{text_content}</div>

        <div style="
            display: flex; justify-content: space-between; align-items: center;
            background: rgba(255,255,255,0.05); padding: 10px 15px; border-radius: 8px;
        ">
            <div style="font-size: 12px; color: #64748b;">Generated by AI-Option</div>
            <div style="font-size: 12px; color: #3b82f6; font-weight: bold;">www.ai-option.com</div>
        </div>
    </div>
    """
    ...
    <body>
        ...
        <button class="share-btn" id="{btn_id}" onclick="generateAndShare()">
            <i class="fas fa-share-square" style="margin-right:5px;"></i> 分享给好友
        </button>

        <div id="modal-overlay" onclick="closeModal()">
            <div id="close-btn">&times;</div>
            <img id="modal-img" src="" />
            <div id="modal-tip">👆 长按图片 -> 转发给朋友</div>
        </div>
        ...
        function generateAndShare() {{
            ...
            html2canvas(target, {{
                ...
                canvas.toBlob(function(blob) {{
                    ...
                    // 1. 尝试调用原生分享 (Navigator Share API)
                    // 要求: 浏览器支持 files 参数 (iOS 15+, Android Chrome)
                    if (navigator.canShare && navigator.canShare({{ files: [file] }})) {{
                        navigator.share({{
                            files: [file],
                            title: '爱波塔 AI 分析',
                            text: '刚刚用 AI 分析了一下行情，来看看结论 👇'
                        }})
                        .then(() => {{
                            resetBtn(btn, originalText, true);
                        }})
                        .catch((error) => {{
        ...
        function showFallbackModal(imgData) {{
        ...
        function resetBtn(btn, originalText, success) {{
            if(success) {{
                btn.innerHTML = '<i class="fas fa-check"></i> 已调起';
    ...
    """
    # 渲染组件 (高度稍微留一点给弹窗，但弹窗是 fixed 的，所以 height 主要是给按钮留位)
...
def get_agent(current_user="访客"):  # 传入 current_user
    ...
    5. 如果客户问最近某商品的行情或技术面，可以把前面几天的K线都一起分析后给出总结
