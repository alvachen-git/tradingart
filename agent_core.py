    ...
    if is_single_source and not has_chart and not is_complex_task:
        ...
        if is_data_query and not is_analysis_query:
            ...
            cio_prompt = f"""
                ...
                1. 技术面分析以K线为主，均线为辅。
                2. 禁止篡改数据：所有数值需参考【团队报告池】。
                3. 知识要参考{kb_context}，但要根据当下市场情况，自己理解后输出
                4. 中国的股票没有期权，客户问股票时，不要给期权策略，除非是用ETF期权来对冲股票。
                5. 如果某品种有利好消息但却下跌，要提醒利多不涨，可能反转，而如果有坏消息但却不跌，要提醒利空不跌，可能阶段底部到了。
                6. 如果记忆{mem_context}有客户的持仓或偏好，在报告里可以针对性的写。
